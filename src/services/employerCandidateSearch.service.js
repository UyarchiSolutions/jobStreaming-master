...
//     { $limit: range },
//   ]);
//   const count = await KeySkill.aggregate([
//     {
//       $match: {
//         $and: [
//           // keyskillSearch,
...
const create_saved_folder = async (req) => {
  let { foldername, candidate } = req.body;
  let userId = req.userId;
  let folder = await Folder.create({ userId: userId, folderName: foldername });
...
const existing_saved_folder = async (req) => {
  let { folder, candidate } = req.body;
  let userId = req.userId;
  let folders = await Folder.findById(folder);
  if (!folders) {
    throw new ApiError(httpStatus.NOT_FOUND, 'Folder Not Found');
  }
  if (folders.userId != userId) {
    throw new ApiError(httpStatus.NOT_FOUND, 'Folder Not Available');
  }
  return new Promise(async (resolve, reject) => {
    let saved = [];
    for (let i = 0; i < candidate.length; i++) {
...
const get_my_folder = async (req) => {
  let folders = await Folder.find({ userId: req.userId });
  return folders;
}
