...
const getAllDistrictDetails = catchAsync(async (req, res) => {
  const district = await DistrictService.getAllDistrict(req.params.key);
  if (!district) {
    throw new ApiError(httpStatus.NOT_FOUND, 'Dsitrict_Details not found');
  }
  res.send(district);
...
const getAllDistrict_all = catchAsync(async (req, res) => {
    const district = await DistrictService.getAllDistrict_all();
    if (!district) {
      throw new ApiError(httpStatus.NOT_FOUND, 'Dsitrict_Details not found');
    }
    res.send(district);
